...
model_list = ["noisy sorting"] # list of simulated models
delta = 1/3
...
plt.suptitle("Nosiy Sorting "+ r'$1/2+\gamma=0.9,\ n=100,\ p=0.2$')
plt.subplot(121)
mean_list = sr_mean_dict["noisy sorting"]
sd_list = sr_sd_dict["noisy sorting"]
...
mean_list = rg_mean_dict["noisy sorting"]
sd_list = rg_sd_dict["noisy sorting"]
plt.plot(L_list, mean_list, label="regularized rank centrality", marker='.', linewidth=1)
plt.fill_between(L_list, np.array(mean_list)-np.array(sd_list), np.array(mean_list)+np.array(sd_list), alpha=0.1)
mean_list = bc_mean_dict["noisy sorting"]
sd_list = bc_sd_dict["noisy sorting"]
...
plt.savefig('./fig/compare_L_borda_ns.png', dpi=800)
